...
def plot_roc_curve(predictions_df):
    # Extract the 'Real' and 'Prob' columns
    ground_truth_values = predictions_df['edge'].tolist()
    probabilities = predictions_df['prob'].tolist()  # Use 'Prob' column

    # Calculate the ROC curve
    fpr, tpr, _ = roc_curve(ground_truth_values, probabilities)

    # Calculate the AUC (Area Under the Curve)
    roc_auc = roc_auc_score(ground_truth_values, probabilities)
...
def create_plots(df):
    ...
    plot_confusion_matrix(df, 'edge', 'pred_edge')
    st.pyplot()
...
if uploaded_file is not None:
    ...
    if predictions is not None:
        # max_value = max([max(predictions['first_node']), max(predictions['first_node'])]))
